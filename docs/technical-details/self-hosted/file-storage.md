...
- **`REPORT_STORAGE_S3_ENDPOINT`** pointing at the in-cluster RustFS service (for example `http://<release>-fw-rustfs-svc:9000`, depending on `rustfs.nameOverride` and release name)
...
When `rustfs.install` is true, the chart can run an optional **create-bucket** Job (`rustfs.createBucketJob`, enabled by default) that uses a short-lived `aws-cli` pod against the in-cluster endpoint. So that the bucket exists before you rely on the app, prefer:
