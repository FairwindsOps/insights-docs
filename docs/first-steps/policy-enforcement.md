...
2.  Setup Insights in your CI/CD pipeline
3.  Create Automation Rules to fine-tune your policy enforcement behaviors in CI and at time of Admission
...
Create a `settings.yaml` file with the following:
```yaml
checks:
  polaris:
    ...
    tagNotSpecified:
      admission:
        block: true
```
Then, use the Insights CLI to push the `settings.yaml` file to Insights. [Learn more about using the Insights CLI to manage Policy settings here.](https://insights.docs.fairwinds.com/configure/cli/settings/#policies-configuration-with-the-cli)
...
Update your `settings.yaml` file with the following:
```yaml
checks:
  polaris:
    memoryRequestsMissing:
