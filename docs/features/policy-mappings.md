...
- `block` is `null` or not present (default) - Insights will only block if the Action Item has a severity of High or Critical - in the case of Kyverno policies managed by Insights, it keeps the behaviour from the original policy
